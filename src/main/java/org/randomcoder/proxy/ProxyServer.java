...
public class ProxyServer {
  ...
  private static final String DEFAULT_AUTH =
      "/srv/randomcoder-proxy/etc/passwd";
   ...
   * @param forward    whether to do X-Forwarded-{For,Proto} handling
   * @param forceHttps whether to force a redirect to HTTPS
   * @param sendSts    whether to send Strict-Transport-Security header
   * @param stsMaxAge  maximum age of STS header
   * @throws IOException if an error occurs
   */
  public ProxyServer(String host, int port, String context, File passwdFile,
      boolean forward, boolean forceHttps, boolean sendSts, long stsMaxAge)
      throws IOException {
  ...
  public static void main(String[] args) throws Exception {
    ...
    if (params.size() > 0) {
      ...
      if ("encrypt".equals(command) && params.size() == 1) {
        ...
        System.out.println(username + ":" + UnixCrypt
            .crypt(new String(password), buf.toString()));
        return;
      }
      usage();
      return;
    ...
    int port = Integer.parseInt(
        System.getProperty(PORT_PARAM, Integer.toString(DEFAULT_PORT)));
    String context = System.getProperty(PATH_PARAM, DEFAULT_PATH);
    String passwd = System.getProperty(AUTH_PARAM, DEFAULT_AUTH);
    boolean forwarded = Boolean.parseBoolean(
        System.getProperty(HTTP_FORWARDED_PARAM, DEFAULT_HTTP_FORWARDED));
    boolean forceHttps = Boolean.parseBoolean(
        System.getProperty(HTTPS_FORCED_PARAM, DEFAULT_HTTPS_FORCED));
    long stsMaxAge = Long.parseLong(
        System.getProperty(STS_MAX_AGE_PARAM, DEFAULT_STS_MAX_AGE), 10);
    boolean sendSts = (stsMaxAge > 0L);

    File passwdFile = new File(passwd);
    if (!passwdFile.exists()) {
      System.err.println("Error: Password file " + passwdFile.getAbsolutePath()
          + " does not exist.");
      return;
    }

    final ProxyServer proxy =
        new ProxyServer(host, port, context, passwdFile, forwarded, forceHttps,
            sendSts, stsMaxAge);
  ...
  public static void usage() {
    ...
    System.err.println();
    System.err
        .println("  If no command is entered, the proxy server will start up.");
    System.err.println();
    System.err.println("  Supported java properties:");
    System.err.println(
        "    " + HOST_PARAM + " -- hostname to listen on [" + DEFAULT_HOST
            + "]");
    System.err.println(
        "    " + PORT_PARAM + " -- port to listen on [" + DEFAULT_PORT + "]");
    System.err.println(
        "    " + PATH_PARAM + " -- context path [" + DEFAULT_PATH + "]");
    System.err.println(
        "    " + AUTH_PARAM + " -- password file [" + DEFAULT_AUTH + "]");
    System.err.println("    " + HTTP_FORWARDED_PARAM
        + " -- whether to honor X-Forwarded-{For,Proto} headers ["
        + DEFAULT_HTTP_FORWARDED + "]");
    System.err.println(
        "    " + HTTPS_FORCED_PARAM + " -- force HTTPS redirects ["
            + DEFAULT_HTTPS_FORCED + "]");
    System.err.println("    " + STS_MAX_AGE_PARAM
        + " -- max age for Strict-Transport-Security header [0 (disabled)]");
  }
}
